    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        if( $this->enable_chunked === FALSE ){
	       ...
	       if ($this->toBytes(ini_get('post_max_size')) < $this->sizeLimit ||
	            ...
	            return array(
	            	...
	            	'error' 	=> array( 
	            		'code' => 100,
	            		'message' => __( "Server Error. Max file size too high, try activate chunking.", "prso-frm-plupload") 
	            	)
	            );
	        }
        ...
        if (!is_writable($uploadDirectory)){
            return array(
	            	...
	            	'error' 	=> array( 
	            		'code' => 100,
	            		'message' => __( "Server error. Uploads directory isn't writable or executable.", "prso-frm-plupload") 
	            	)
	            );
        }
        ...
        if(!isset($_SERVER['CONTENT_TYPE'])) {
            ...
            return array(
            	...
            	'error' 	=> array( 
            		'code' => 100,
            		'message' => __( "Server error. Not a multipart request. Please set forceMultipart to default value (true).", "prso-frm-plupload") 
            	)
            );
        }
        ...
        if ($name === null || $name === ''){
            return array(
            	...
            	'error' 	=> array( 
            		'code' => 100,
            		'message' => __( "File name is empty.", "prso-frm-plupload") 
            	)
            );
        }
        ...
        if ($size == 0){
            return array(
            	...
            	'error' 	=> array( 
            		'code' => 100,
            		'message' => __( "File is empty.", "prso-frm-plupload") 
            	)
            );
        }
        ...
        if ($size > $this->sizeLimit){
            return array(
            	...
            	'error' 	=> array( 
            		'code' => 100,
            		'message' => sprintf( __("File is too large. Max %s M", "prso-frm-plupload"), $this->sizeLimit )
            	)
            );
        }
        ...
        if ($totalParts > 1){
            ...
            if (!is_writable($chunksFolder)){
                return array(
	            	...
	            	'error' 	=> array( 
	            		'code' => 100,
	            		'message' => __( "Server error. Chunks directory isn't writable or executable.", "prso-frm-plupload")
	            	)
	            );
            }
        	...
        	if ($out) {
				...
				if ($in) {
					...
					return array(
		            	...
		            	'error' 	=> array( 
		            		'code' => 100,
		            		'message' => __( "Failed to open input stream", "prso-frm-plupload")
		            	)
		            );
				}
				...
				return array(
		            	...
		            	'error' 	=> array( 
		            		'code' => 100,
		            		'message' => __( "Failed to open chunk destination file", "prso-frm-plupload")
		            	)
		            );
			}
	            ...
	            if( isset($file_info['file_path']) ) {
		            ...
		            if( $this->move_file($tmp_chunk_file_path, $target) ) {
			            ...
			            return array(
			            	...
			            	'error' 	=> array( 
			            		'code' => 100,
			            		'message' => __( "Failed to move final buffer file", "prso-frm-plupload")
			            	)
			            );
		            }
		            ...
		            return array(
			            	...
			            	'error' 	=> array( 
			            		'code' => 100,
			            		'message' => __( "Error generating final file path", "prso-frm-plupload")
			            	)
			            );
	            }
            ...
            return array(
            	...
            	'error' 	=> array( 
            		'code' => 100,
            		'message' => __( "The upload was cancelled, or server error encountered", "prso-frm-plupload")
            	)
            );
        }
    }
	...
	protected function validateUploadedFile( $name = NULL, $file_path = NULL ) {
        ...
        if($this->allowedExtensions && !in_array(strtolower($ext), array_map("strtolower", $this->allowedExtensions))){
            ...
            return array(
            	...
            	'error' 	=> array( 
            		'code' => 100,
            		'message' => sprintf( __("File has an invalid extension, it should be one of %s.", "prso-frm-plupload"), $these )
            	)
            );
        ...
        if( !empty($mime_type) && !in_array($mime_type, array_values($this->allowed_mimes)) ) {
	        ...
	        return array(
            	...
            	'error' 	=> array( 
            		'code' => 100,
            		'message' => sprintf( __("File Type Error: %s.", "prso-frm-plupload"), $mime_type )
            	)
            );
