 ...
 *	Also updates the Formidable entry view to show links to file at video service e.g. Youtube.
 *
 *	Actions:
...
class PrsoAdvVideoUploader {
	...
	*/
	...
	protected	$plugin_options_slug	= PRSOFRMADVUPLOADER__OPTIONS_NAME;
	...
	*/
	public function add_actions() {
		...
		//Hook into formidable after submission hook to fire actions
		//After a form has been submitted succesfully
		//add_action( 'prso_frm_pluploader_processed_uploads', array( $this, 'process_wp_attachments' ), 10, 3 );
		add_action( 'prso_frm_pluploader_processed_uploads', array( $this, 'background_init' ), 10, 3 );
		
		//Add wp ajax hook for method to init processing wp attachments
		add_action( 'wp_ajax_nopriv_prso_frm_youtube_upload_init', array($this, 'init_attachment_process') );
		
		add_action( 'wp_ajax_nopriv_prso_frm_youtube_upload_save_data', array($this, 'save_video_data') );
		
		//Add custom script to formidable enqueue
		add_action( 'frm_enqueue_form_scripts', array($this, 'frm_enqueue_scripts'));
	...
	*/
	public function add_filters() {
		...
		//Filter links to attachments in frm entry
		add_filter( 'prso_frm_pluploader_entry_attachment_links', array($this, 'get_video_link'), 10, 3 );
			
		//Formidable filter for submit button render
		add_filter( 'frm_form_replace_shortcodes', array($this, 'frm_submit_button'), 1, 3 );
	...
	* background_init
	* 
	* Called By Action: prso_frm_pluploader_processed_uploads
	* 
	* Hooks into custom action for Formidable Advanced Uploader plugin.
	* Once all attachments have been processed by the plugin and added to wordpress
	* media library. 
	*
	* The array of attachments, Formidable Entry & Form data are prepared to be sent
	...
	*/
	public function background_init( $wp_attachment_data, $entry, $form ) {
		...
		if( $plugin_options !== FALSE && isset($plugin_options['api_select']) ) {
			...
			$ajax_hook_slug = 'prso_frm_youtube_upload_init';
			...
			//Set the entry array from formidable
			$fields['entry'] = urlencode( json_encode($entry) );
			
			
			//Set form array from formidable
	...
	* 
	* Called by WP Ajax Action: prso_frm_youtube_upload_init
	*
	...
	*/
	public function init_attachment_process() {
		...
		if( isset($_POST['api'], $_POST['wp_attachment_data'], $_POST['entry'], $_POST['form']) ) {
			...
			//Cache entry id and form id passed from formidable
			$entry	=  urldecode( $_POST['entry'] );
			$entry	=  json_decode( $entry, TRUE );
			
			//Decode form array from formidable
	...
	*/
	public function process_wp_attachments( $wp_attachment_data, $entry, $form ) {
		...
		if( !empty($wp_attachment_data) && !empty($entry) ) {
			...
			//Cache entry and form data from formidable
			$this->data['frm_entry'] = $entry;
			$this->data['frm_form']	= $form;
			...
			if( is_array($wp_attachment_data) ) {
				...
				foreach( $wp_attachment_data as $field_id => $wp_attachments ) {
					...
					foreach( $wp_attachments as $key => $attachment_id ) {
						...
						do_action( 'prso_frm_youtube_uploader_pre_get_attachment_data', $wp_attachment_data, $field_id, $attachment_id );
			...
			$this->plugin_error_log( 'Main Plugin:: wp attachment data or frm entry data empty' );
		}
	...
	*/
	private function generate_file_titles( $validated_attachments ) {
		...
		//Check that our global data from formidable is available
		if( isset($this->data['frm_entry']['id'], $this->data['frm_entry']['form_id']) ) {
			
			$form_id 	= (int) $this->data['frm_entry']['form_id'];
			$entry_id	= (int) $this->data['frm_entry']['id'];
			...
			foreach( $validated_attachments as $field_id => $attachments ) {
				...
				foreach( $attachments as $key => $attachment_data ) {
					...
					$title = sprintf( __('Uploaded File: Form %1$s, Entry %2$s, File %3$s', 'frm-youtube-upload'), 
						$form_id,
						$entry_id,
						$key
					);
					
					$description = sprintf( __('This video was uploaded from your website, here are the details: Form %1$s, Entry %2$s, File %3$s', 'frm-youtube-upload'), 
						$form_id,
						$entry_id,
						$key
					);
	...
	* 
	* Calls Ajax Action: prso_frm_youtube_upload_save_data
	*
	...
	*/
	private function background_save_data( $upload_result_data ) {
		...
		//Cache the slug of our wp ajax hook to run our process
		$ajax_hook_slug = 'prso_frm_youtube_upload_save_data';
		...
		//Json encode and cache formidable entry array into post
		$postdata['frm_entry']	= urlencode( json_encode($this->data['frm_entry']) );

		//Json encode and cache formidable Form array into post
		$postdata['frm_form']	= urlencode( json_encode($this->data['frm_form']) );
	...
	* save_video_data
	* 
	* Called by Ajax Action: 'prso_frm_youtube_upload_save_data'
	*
	...
	*/
	public function save_video_data() {
		...
		//Json decode formidable entry array
		$this->data['frm_entry'] = urldecode( $_POST['frm_entry'] );
		$this->data['frm_entry'] = json_decode( $this->data['frm_entry'], TRUE );
		
		//Json decode formidable Form array
		$this->data['frm_form'] = urldecode( $_POST['frm_form'] );
		$this->data['frm_form']	= json_decode( $this->data['frm_form'], TRUE );
		...
		if( isset($original_wp_attachments) && !empty($upload_result) ) {
			...
			//Update formidable data for this entry
			$this->update_frm_entry_meta( $original_wp_attachments );
	...
	* @author	Ben Moody
	*/
	private function update_frm_entry_meta( $field_values = array() ) {
		...
		if( !empty($field_values) && isset($this->data['frm_entry']['id']) ) {
			
			//Cache entry data provided from formidable
			$entry_id = $this->data['frm_entry']['id'];
			
			//Allow devs to hook before we get the form table names ect
			do_action('prso_frm_youtube_uploader_pre_update_meta');
			
			//Loop the array of field values and update the formidable meta for each field
			foreach( $field_values as $field_id => $value ) {
    	        ...
    	        //Update the frm meta values with our new data including the video api data
    	        if ( empty($results) ) {
	...
	* @author	Ben Moody
	*/
	public function frm_enqueue_scripts( $form ) {
		...
		if ( ! is_admin() || defined('DOING_AJAX') ) {
			...
			wp_enqueue_script( 'prso-frm-api-upload', plugins_url('js/frm-api-upload.js', __FILE__) );
			
			//Form plugin js object
			$plugin_script_obj['frm_form_id'] = $form_id; 
			$plugin_script_obj['images'] 	= plugins_url('images', __FILE__);
			
			//Localize plugin js object
			wp_localize_script( 'prso-frm-api-upload', 'prso_frm_api_upload_vars', $plugin_script_obj );
	...
	* frm_submit_button
	* 
	* Called by Formidable Filter: frm_form_replace_shortcodes
	*
	...
	* @author	Ben Moody
	*/
	public function frm_submit_button( $html, $form, $values ) {
		
		//Init vars
		$plugin_images_folder_url 	= NULL;
		$loading_html				= NULL;
		$wait_text                  = __( 'Uploading files please wait, may take up to 5 min.', 'frm-youtube-upload' );
		
		// Make sure it's a submit button
		if ( ! strpos($html, '[button_action]') && ! strpos($html, '[if save_draft]') && ! strpos($html, '[button_label]') ) {
            return;
        }
		
		//Cache html for submit loading
		$loading_html = "<div style='display:none;' class='frm-api-uploader'><p>{$wait_text}</p></div>";
		
		//Append to submit button html before closing div
		if ( preg_match('/\<\/div\>$/', $html) ) {
		    $html = preg_replace('/\<\/div\>$/', $loading_html .'</div>', $html);
		} else {
		    $html .= $loading_html;
		}
