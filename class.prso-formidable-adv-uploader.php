<?php
class PrsoFrmAdvUploader {
	
	protected static $class_config 				= array();
	protected $current_screen					= NULL;
	protected $plugin_ajax_nonce				= 'prso_frm_adv_uploader-ajax-nonce';
	...
	function __construct( $config = array() ) {
		...
		$PluginInit = new PrsoFrmAdvUploaderInit();
	...
	*
	* Filters: 
	*	'prso_frm_pluploader_shortcode_attach_title' - filter individual attachment title
	*	'prso_frm_pluploader_shortcode'	- Filter shortcode output
	*
	* @access 	public
	* @author	Ben Moody
	*/
	public function adv_post_attachments_shortcode( $attr ) {
		...
		if( !empty($attachments) ) {
			$output = '<ul class="frm-adv-uploader-attachments">';
			foreach( $attachments as $attachment_id => $Attachment ) {
				
				$attachment_title = apply_filters( 'prso_frm_pluploader_shortcode_attach_title', $Attachment->post_title . ' ('. $Attachment->post_mime_type .')', $Attachment );
		...
		return apply_filters( 'prso_frm_pluploader_shortcode', $output, $attachments );
	}
